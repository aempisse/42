...
int				ft_burning_ship_loop(t_env *env, long int x, long int y)
{
	...
	while (((sq.r + sq.i) < 4) && ++loop < env->loop)
	{
		...
		sq.i = z.i * z.i;
		tmp = z.r;
		z.r = ABS(sq.r - sq.i + c.r);
		z.i = ABS(2 * tmp * z.i + c.i);
	}
	return (loop);
}
	...
	while (pt.y++ < max.y)
	{
		...
		while (pt.x++ < max.x)
		{
			ft_put_image(env->img, (env->img->width * (pt.y - env->offset.y) + 
				...
				env->color(ft_burning_ship_loop(env, pt.x, pt.y), env->loop));
		}
	}
}
