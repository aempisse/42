...
void			draw_value(t_env *env)
{
	if (ft_strcmp(env->control->name_obj, "Sphere") == 0)
	{
		...
		mlx_string_put(env->mlx, env->win_control, 205, 100, 0xFFFFFFF, ft_itoa(env->objects->spheres->length - 1));
	...
	if (ft_strcmp(env->control->name_obj, "Plane") == 0)
	{
		...
		mlx_string_put(env->mlx, env->win_control, 205, 100, 0xFFFFFFF, ft_itoa(env->objects->planes->length - 1));
